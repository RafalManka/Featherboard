...
**Stack:** `axum` + `sqlx`/SQLite (single database for both self-hosted and hosted deployments) + `askama`/`minijinja` server-rendered templates + `htmx` for interactivity (no client-side framework). Billing via Stripe Checkout + webhooks. Email via `lettre`/SMTP. Ships as a single binary — no Docker or Postgres required to self-host.
